...
class Cell(models.Model):
    ...
    second_in_cmd = models.ForeignKey(CustomUser, on_delete=models.SET_NULL, null=True, blank=True,
                                      related_name='second_in_cmd_cell')
...
class CellMember(models.Model):
    ...
    cell        = models.ForeignKey(Cell, on_delete=models.CASCADE, related_name='cell_members')
    user        = models.ForeignKey(CustomUser, on_delete=models.CASCADE, related_name='cell_memberships')
    attendance  = models.CharField(max_length=20, choices=ATTENDANCE_CHOICES, default='regular')
    ...
    def __str__(self):
        return f"{self.user.get_full_name()} – {self.cell.name} ({self.attendance})"
...
class CellMeetingReport(models.Model):
    ...
    facilitated_by = models.ForeignKey(CustomUser, on_delete=models.CASCADE,
                                        related_name='facilitated_reports')
